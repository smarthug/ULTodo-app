...
export function PomodoroPage() {
  ...
  return (
    <div className="flex min-h-full flex-col items-center justify-between overflow-hidden bg-paper px-5 py-8 text-center">
      <div>
      ...
      <motion.div animate={{ scale: timer.running ? [1, 1.03, 1] : 1 }} transition={{ repeat: timer.running ? Infinity : 0, duration: 4 }} className="relative grid size-72 place-items-center rounded-full bg-[radial-gradient(circle,var(--accent-soft),transparent_68%)]">
        ...
        <div className="relative">
          <div className="font-mono text-[54px] font-semibold tracking-[-.06em] text-ink">{fmt(timer.remaining)}</div>
          <p className="mt-2 max-w-52 text-sm font-medium text-ink-3">{task?.title ?? 'Pick a focus task from Today'}</p>
        </div>
      </motion.div>
